...
import { MESSAGE_MAX_CHARS, MAX_FILES_PER_MESSAGE } from '../config.js';
import { quotaPrecheck, enforceQuota } from '../quota.js';
import { canInteract } from '../social.js';
import {
  uploadVoice,
  uploadAttachment,
  publicPathFor,
...
router.get('/:peerId', authRequired, (req, res) => {
  ...
  // В private-режиме история доступна только по существующей связи.
  if (!canInteract(me, peerId)) return res.status(403).json({ error: 'not allowed' });
  const rows = db
    .prepare(
      `SELECT ${MSG_COLS}
       FROM messages
...
router.post(
  ...
  (req, res) => {
    ...
    if (!canInteract(req.user.id, to)) return res.status(403).json({ error: 'not allowed' });
...
router.post(
  ...
  authRequired,
  quotaPrecheck,
  uploadAttachment.array('files', MAX_FILES_PER_MESSAGE),
  enforceQuota,
  sniff(),
  (req, res) => {
    const to = Number(req.body.to);
    ...
    const caption =
      typeof req.body.content === 'string'
        ? req.body.content.trim().slice(0, MESSAGE_MAX_CHARS)
        : '';
    const rawReply = req.body.replyToId;
    const replyToIdRaw = rawReply === '' || rawReply === undefined ? null : Number(rawReply);
    const replyCheck = validateReplyTo(replyToIdRaw, { me: req.user.id, peerId: to });
...
router.patch('/:id', authRequired, (req, res) => {
  ...
  if (trimmed.length > MESSAGE_MAX_CHARS) return res.status(400).json({ error: 'too long' });
