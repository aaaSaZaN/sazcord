...
// Linux Wayland & PipeWire screen capture support (KDE Plasma & GNOME)
...
// Sazcord, две tray-иконки и два mainProcess'а с конфликтом за audio /
// configFile.
//
...
// — там `@sazcord/desktop` — и кладёт пользовательские данные в
// `%APPDATA%\@sazcord\desktop`, а в собранном NSIS-приложении —
// в `%APPDATA%\Sazcord` (productName из electron-builder). Из-за
// разных userData renderer'у достаются РАЗНЫЕ localStorage между
// dev и прод-сборкой, и юзер видит «разлогин на каждом запуске».
// setName('Sazcord') синхронизирует обе ветки.
app.setName('Sazcord');

// Dev-режим (`electron .` без packaging) кладём в отдельный профиль
// `Sazcord-dev`, чтобы:
//   1) если у юзера в фоне крутится установленный prod-Sazcord, его
...
//      файлы профиля.
if (!app.isPackaged) {
  app.setPath('userData', path.join(app.getPath('appData'), 'Sazcord-dev'));
}
...
function createWindow() {
  ...
  // Полный доступ к медиаустройствам для веб-клиента
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    // serverUrl прокидываем, чтобы фид обновлений жил на том же хосте,
    // что и сам инстанс Sazcord (юзер может сменить адрес в настройках).
    autoUpdater.setup(mainWindow, { app, ipcMain, serverUrl: cfg.serverUrl });
  });
...
function createTray() {
  ...
  tray.setToolTip('Sazcord');
  const menu = Menu.buildFromTemplate([
    { label: 'Открыть Sazcord', click: () => showWindow() },
    { type: 'separator' },
...
// при логине Sazcord запускался сразу свернутым в трей.
//
...
<html><head>
<meta charset="utf-8">
<title>Sazcord — настройка</title>
<style>
  body{margin:0;background:#070a1a;color:#e2e8f0;font-family:system-ui,Segoe UI,Helvetica,Arial,sans-serif;display:grid;place-items:center;min-height:100vh}
  ...
  input{width:100%;box-sizing:border-box;padding:10px 12px;background:#070a1a;border:1px solid #232830;border-radius:8px;color:#e2e8f0;font-size:14px}
...
<div class="card">
  <h1>Адрес сервера Sazcord</h1>
  ${errBlock}
  <label>URL (например, https://sazcord.example.com)</label>
...
ipcMain.handle('config:set', (_e, patch) => {
  ...
  config.save(cfg);
  // serverUrl изменился — перезагрузим окно и перенастроим апдейтер:
  // фид обновлений должен переехать на новый хост вместе с клиентом.
  if ('serverUrl' in patch && mainWindow) {
    autoUpdater.reset(ipcMain);
    autoUpdater.setup(mainWindow, { app, ipcMain, serverUrl: cfg.serverUrl });
...
ipcMain.handle('media:request-permission', async (_e, type) => {
...
ipcMain.handle('media:get-status', (_e, type) => {
  if (process.platform === 'darwin') {
    return systemPreferences.getMediaAccessStatus(type === 'camera' ? 'camera' : 'microphone');
  }
  return 'granted';
});
...
app.whenReady().then(async () => {
  // Автоматически разрешаем доступ к медиа (микрофон/камера) внутри окна Electron
  if (session.defaultSession) {
    session.defaultSession.setPermissionRequestHandler((webContents, permission, callback) => {
      if (['media', 'microphone', 'camera', 'notifications', 'audioCapture', 'display-capture'].includes(permission)) {
  ...
  // На macOS запрашиваем системное разрешение на микрофон и камеру
  if (process.platform === 'darwin') {
    try {
  ...
  // Клик по dock-иконке на macOS. Проверять «окон нет» здесь нельзя:
  // крестик у нас не закрывает окно, а прячет его (closeToTray), так что
  // окно существует — просто невидимое, и приложение переставало
  // разворачиваться. showWindow() и показывает скрытое, и пересоздаёт
  // уничтоженное.
  app.on('activate', () => {
    showWindow();
  });
...
//   - macOS / Linux → audio: undefined. 'loopback' в Electron реализован
//     только под Windows; на Linux он приводит к отказу всего запроса
//     getDisplayMedia (демонстрация просто не стартует), на macOS OS не
//     даёт системный loopback. Чекбокс «звук» в picker'е там выключен.
//
// Тонкости:
  ...
  session.defaultSession.setDisplayMediaRequestHandler(
    async (request, callback) => {
      try {
        ...
        // Source берём из кэша picker'а (он только что его показывал).
        //
        // Повторный desktopCapturer.getSources() тут был багом на Linux:
        // с Wayland/PipeWire каждый вызов заново открывает
        // xdg-desktop-portal и выдаёт новые id, поэтому find() по
        // выбранному id ничего не находил → callback({}) → «выбрал экран,
        // а демонстрация не начинается». Fallback на свежий запрос
        // оставляем только если кэш пуст (X11/старые сборки).
        let source = screenPicker.getCachedSource(choice.id);
        if (!source) {
          const sources = await desktopCapturer.getSources({
            types: ['screen', 'window'],
            thumbnailSize: { width: 1, height: 1 },
          });
          source = sources.find((s) => s.id === choice.id) || null;
        }
        if (!source) {
        ...
        // Audio-mode для callback'а. 'loopback' поддерживается ТОЛЬКО на
        // Windows (см. docs Electron: setDisplayMediaRequestHandler →
        // audio: 'loopback' | 'loopbackWithMute'). На Linux Chromium
        // отклоняет такой callback целиком, и getDisplayMedia падает —
        // это и была причина «на Linux демонстрация не запускается».
        // На macOS системный звук тоже недоступен.
        let audioMode = undefined;
        ...
        if (
          ...
          processAudio.capture.isSupported()
        ) {
...
//     которые Sazcord может запускать) — но это редкий кейс.
//
...
//
// process.execPath = полный путь до Sazcord.exe в установленной папке
// (например, C:\Users\Username\AppData\Local\Programs\Sazcord\Sazcord.exe).
