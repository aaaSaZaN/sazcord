...
// клиенту Sazcord. Любая попытка из renderer'а дотянуться до Node API
...
//   событие в DOM-event 'sazcord:shortcut' с detail.action = 'toggleMute'
...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // Используется в SettingsPanel для отображения "Sazcord X.Y.Z" в сайдбаре.
  // Возвращает Promise<string>.
  getVersion: () => ipcRenderer.invoke('app:version'),

  requestMediaPermission: (type) => ipcRenderer.invoke('media:request-permission', type),
  getMediaStatus: (type) => ipcRenderer.invoke('media:get-status', type),
  ...
  onShortcut: (handler) => {
    const listener = (_e, payload) => {
      try {
        ...
        window.dispatchEvent(new CustomEvent('sazcord:shortcut', { detail: { action } }));
  ...
  onUpdateEvent: (handler) => {
    const listener = (_e, payload) => {
      try {
        ...
        window.dispatchEvent(new CustomEvent('sazcord:update', { detail: payload }));
