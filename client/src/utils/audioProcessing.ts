 ...
 *                                       └── setInterval-цикл считает RMS и
 *                                           двигает gateGain.gain (0/1
 *                                           с плавными переходами). НЕ rAF:
 *                                           rAF стоит в скрытой/перекрытой
 *                                           вкладке и намертво запирал ворота.
 *
...
export async function createMicPipeline(
  ...
  // Носитель «текущих» настроек, мутируется через updateSettings; управляющий
  // цикл читает оттуда без замыкания на старые значения.
  const live = { s };

  // RMS-детектор для ворот. Если ворота выключены — gainTarget=1 всегда.
  //
  // ВАЖНО (регрессия «меня не слышно из браузера»): раньше этот цикл
  // крутился на requestAnimationFrame. rAF полностью замораживается,
  // когда вкладка скрыта ИЛИ окно перекрыто другим окном (occlusion).
  // Если в этот момент ворота были закрыты (gateGain = 0), они так и
  // оставались закрытыми — исходящий трек уходил в вечную тишину, и
  // собеседники переставали слышать веб-юзера, хотя он их слышал.
  // Десктоп (Electron) этим не болел: там pipeline вообще не включается.
  //
  // Теперь цикл живёт на setInterval (таймеры в странице с активным
  // WebRTC/аудио Chrome не режет так агрессивно) + защита от throttling:
  // если между тиками прошло больше GATE_STALL_MS, считаем, что цикл
  // задушен и «открываем» ворота (fail-open) — лучше пропустить шум,
  // чем замьютить человека наглухо.
  const GATE_TICK_MS = 25;
  const GATE_STALL_MS = 250; // тик опоздал => цикл throttled
  const GATE_HEALTHY_TICKS = 3; // столько вовремя пришедших тиков, чтобы снова доверять

  const buf = new Float32Array(gateAnalyser.fftSize);
  let lastAboveTs = 0;
  let lastTickTs = performance.now();
  let healthyTicks = GATE_HEALTHY_TICKS;
  let timerId: any = 0;
  let cancelled = false;

  const setGate = (targetGain: number, rampMs: number) => {
  ...
  const tick = () => {
    ...
    if (!cs.noiseGateEnabled) {
      ...
      healthyTicks = GATE_HEALTHY_TICKS;
      return;
    }

    if (dt > GATE_STALL_MS) {
      // Цикл задушен браузером: анализатор держит всего ~20 мс истории,
      // по такой выборке решать «говорит / не говорит» нельзя. Открываем
      // ворота и ждём, пока таймер снова пойдёт с нормальной частотой.
      healthyTicks = 0;
      lastAboveTs = now;
      setGate(1, cs.noiseGateAttackMs);
      return;
    }
    if (healthyTicks < GATE_HEALTHY_TICKS) {
      healthyTicks += 1;
      lastAboveTs = now;
      setGate(1, cs.noiseGateAttackMs);
      return;
    ...
    setGate(open ? 1 : 0, open ? cs.noiseGateAttackMs : cs.noiseGateReleaseMs);
  };
  timerId = setInterval(tick, GATE_TICK_MS);
...
export function createMicScreenMixer(opts: {
  ...
  if (!Ctx) {
    ...
    // практике все таргеты Sazcord поддерживают AudioContext.
    return {
