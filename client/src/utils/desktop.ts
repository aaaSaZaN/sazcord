...
//   вне фокуса Sazcord — в играх, fullscreen-приложениях и т.п.
...
export type UpdateEvent =
  ...
  // manual:true — apple-специфика: без Developer ID сертификата
  // Squirrel.Mac не умеет ставить апдейт сам, поэтому main отдаёт прямую
  // ссылку на .dmg, а UI предлагает «Скачать» вместо «Перезапустить».
  | {
      kind: 'downloaded';
      version?: string;
      releaseDate?: string;
 ...
 * window-event 'sazcord:shortcut' никогда не диспатчится.
 *
...
/**
 * Подписка на DOM-event 'sazcord:shortcut', который preload.js шлёт в
...
export function onShortcutEvent(handler: (action: ShortcutAction) => void): () => void {
  ...
  window.addEventListener('sazcord:shortcut', listener as EventListener);
  return () => window.removeEventListener('sazcord:shortcut', listener as EventListener);
}
 ...
 * никем не повешен, и window-event 'sazcord:update' никогда не
...
export function onUpdateEvent(handler: (e: UpdateEvent) => void): () => void {
  ...
  window.addEventListener('sazcord:update', listener as EventListener);
  return () => window.removeEventListener('sazcord:update', listener as EventListener);
}
 ...
 */
/**
 * Выйти с сервера: стереть serverUrl в конфиге оболочки.
 *
 * Пустая строка — то же состояние, что у свежей установки, и main-процесс
 * на неё уже реагирует показом экрана первичной настройки
 * (см. loadServerUrl в desktop/main.js). Отдельного IPC не нужно.
 *
 * Сессию чистим здесь, до записи конфига: localStorage принадлежит origin
 * сервера, и после переключения на data:URL достучаться до него уже
 * нельзя — юзер вернулся бы на тот же адрес всё ещё залогиненным.
 */
export async function leaveServer(): Promise<boolean> {
  ...
  try {
    ...
    console.warn('leaveServer failed:', e);
    return false;
  }
}
