...
const ACTIVE_GROUPCALL_STORAGE_KEY = 'sazcord.activeGroupCall';
...
export function useGroupCall({ socket, selfUser, settings, toast, sounds }) {
  ...
  // Офферы, прилетевшие ДО того, как мы закончили join (нет callId и/или
  // ещё не захвачен микрофон). Раньше такие офферы просто отбрасывались:
  // сервер рассылает `groupcall:peer-joined` остальным участникам РАНЬШЕ,
  // чем отдаёт ack джойнящемуся, поэтому инициатор успевал прислать оффер
  // в окно, когда у нас ещё не было callId. Ответ он не получал никогда —
  // и в mesh'е навсегда оставалась дырка: один участник другого не слышит
  // и не видит. Теперь копим и проигрываем после входа в звонок.
  const pendingOffersRef = useRef(new Map()); // userId -> sdp
  // Ссылка на актуальный обработчик оффера (живёт внутри socket-эффекта),
  // чтобы flush мог его вызвать.
  const applyOfferRef = useRef<null | ((p: any) => void)>(null);
  ...
  const cleanup = useCallback(() => {
    // stateRef опускаем СРАЗУ: иначе синхронный сценарий
    // «выйти из звонка → тут же войти в другой» упирается в guard
    // `stateRef.current !== 'idle'` внутри join(), потому что реальный
    // state обновится только на следующем рендере.
      ...
      if (isInitiator) {
        (async () => {
          try {
            ...
            sendOffer();
            // Ретрай оффера: пир мог быть ещё в процессе join'а (у него не
            // было callId) и наш первый оффер ушёл в никуда. Пока мы висим
            // в 'have-local-offer' — повторяем, но не бесконечно.
            let attempts = 0;
  ...
  useEffect(() => {
    ...
    // callId строгой проверки не делаем — сервер может прислать r2 при рестарте.
    // Наличие callId тоже НЕ требуем: сигналинг легально прилетает раньше,
    // чем отработал ack на groupcall:join (см. pendingOffersRef).
    const isMyCall = (payload) => {
      const gid = groupRef.current?.id;
      if (!gid) return false;
      return payload.groupId === gid;
    };
    ...
    const onPeerJoined = ({ groupId, userId }) => {
      ...
      if (!pcsRef.current.has(userId)) {
        const iAmInitiator = selfUser.id < userId;
        createPeerConnection(userId, iAmInitiator);
      } else {
    ...
    const onOffer = async ({ from, sdp, groupId }) => {
      ...
      // Ещё не в звонке (нет callId / нет локальных треков) — откладываем.
      // Создавать pc сейчас нельзя: на sender'ы уехал бы «тихий»
      // placeholder-трек, и пир навсегда остался бы без нашего голоса.
      if (stateRef.current !== 'in-call' || !callIdRef.current) {
  ...
  // Проигрываем отложенные офферы, как только вошли в звонок.
  useEffect(() => {
    if (state !== 'in-call') return;
    const apply = applyOfferRef.current;
    const pending = pendingOffersRef.current;
    if (!apply || pending.size === 0) return;
    const entries = [...pending.entries()];
    pending.clear();
    const gid = groupRef.current?.id;
    for (const [from, sdp] of entries) apply({ from, sdp, groupId: gid });
  }, [state]);
