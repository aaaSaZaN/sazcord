 ...
 * Состояния: 'idle' | 'calling' | 'incoming' | 'connecting' | 'in-call'.
 *
 * Состояния 'waiting' больше НЕТ. Раньше после «Отбоя» звонок на 5 минут
 * подвисал в этом состоянии «вдруг пир вернётся», окно не закрывалось и
 * блокировало вход в групповой звонок. По требованию убрано: положил
 * трубку — звонок закончен у обеих сторон, сразу и окончательно.
 *
...
// Ключ старой (удалённой) персистенции звонка. Оставлен только для того,
// чтобы вычистить записи, оставшиеся у пользователей от прошлых версий.
const ACTIVE_CALL_STORAGE_KEY = 'sazcord.activeCall';

export function useCall({ socket, selfUser, settings, toast, sounds }) {
  // Состояние 'waiting' (5-минутное окно на возврат) УБРАНО по требованию:
  // после «Отбоя» звонок должен просто заканчиваться. Раньше клиент ещё и
  // воскрешал его из sessionStorage после F5, из-за чего окно звонка
  // «залипало» и блокировало вход в групповые звонки.
  ...
  // снимается в cleanup. Помогает не наплодить параллельных
  ...
  const cleanup = useCallback(
    (reason) => {
      // stateRef опускаем СРАЗУ, не дожидаясь ре-рендера: иначе код,
      // который сразу после hangup() пытается зайти в другой звонок
      // (переход в групповой), упрётся в guard `state !== idle`.
  ...
  // Вне звонка (idle/incoming) — следующий start() сам подхватит
  ...
  useEffect(() => {
    ...
    // 'incoming' — мы ещё не приняли (нет своих треков).
    if (curState === 'idle' || curState === 'incoming') {
      return undefined;
    }
    let cancelled = false;
  ...
  const hangup = useCallback(
    (reason) => {
      ...
      void curState;
      // Никакого waiting: положили трубку — звонок закончен у обоих.
      cleanup('hangup');
    },
    [cleanup, socket],
  );
  ...
  const start = useCallback(
    async (targetUser, { withVideo: wantVideo = false } = {}) => {
      ...
      if (state !== 'idle' && state !== 'waiting') {
        // Раньше тут был молчаливый return: юзер жал «Позвонить» в профиле
        // и вообще ничего не происходило («будто не звонится»). Объясняем.
        toast?.info?.(
          state === 'incoming'
            ? 'Сначала ответьте на входящий звонок или отклоните его'
            : 'Вы уже в звонке — сначала завершите текущий',
        );
        return;
      }
  ...
  const accept = useCallback(async () => {
    ...
    try {
      ...
      socket.emit('call:accept', { to: targetPeer.id, callId: cid });
  ...
  }, [createPeerConnection, getLocalMedia, hangup, socket, sounds, toast, withVideo]);
  ...
  // даже когда окно Sazcord не в фокусе — для этого мы тут вешаемся
  // на DOM-event 'sazcord:shortcut', который шлёт preload.js. На вебе
    ...
    async (presetKey = '720p', includeAudio = false) => {
      ...
      try {
        ...
        if ((display as any).windowAudioStripped) {
          toast?.info?.(
            'В браузере звук одного окна захватить нельзя. Откройте Sazcord для десктопа или поделитесь экраном целиком, чтобы передавать звук.',
          );
        }
  ...
  useEffect(() => {
    ...
    const onInvite = ({
      ...
      // Если мы активно разговариваем (in-call или connecting с активным pc) — отклоняем как busy.
      if (pcRef.current || curState === 'in-call' || (curState === 'connecting' && pcRef.current)) {
        socket.emit('call:reject', { to: from, callId, reason: 'busy' });
        return;
      }

      // Если висел старый waiting-звонок — очистим его таймеры
      if (curState === 'waiting') {
        cleanup(undefined);
      }
    ...
    const onOffer = async ({ from, callId, sdp }) => {
      ...
      if (!pcRef.current) {
        // accept ещё не нажат или PC ещё создаётся — сохраним
      ...
      try {
        ...
        console.warn('onOffer handling error', e);
      }
    };
    ...
    const onAnswer = async ({ from, callId, sdp }) => {
      ...
      try {
        ...
        console.warn('onAnswer handling error', e);
      }
    };
    ...
    const onAccept = () => {
      sounds?.stopOutgoing?.();
      setState((s) => (s === 'calling' ? 'connecting' : s));
      // Повторно отсылаем offer, если он ещё не дошёл до callee
      if (pcRef.current?.localDescription && peerRef.current && callIdRef.current) {
    ...
    // Звонок приняли/отклонили на другом устройстве этого же аккаунта.
    // Гасим рингтон и закрываем модалку здесь — иначе на ноуте продолжает
    // звонить входящий, хотя разговор уже идёт с телефона.
    const onHandled = ({ callId, action }) => {
      if (callIdRef.current && callIdRef.current !== callId) return;
      ...
      toast?.info?.(
        action === 'accepted'
          ...
          : 'Звонок отклонён на другом устройстве',
      );
      cleanup(undefined);
    };
    ...
    const onEnd = ({ reason }) => {
      ...
      // Эхо-защита: при F5 у пира сервер шлёт сначала peer-leaving
      // (beforeunload), затем peer-disconnected (disconnect handler).
      // Второй прилетает, когда мы уже в idle — просто игнорируем.
      if (temporary && curState === 'idle') return;
      if (reason === 'peer-disconnected' || reason === 'peer-leaving') {
        toast?.info?.('Собеседник отключился');
      } else if (reason === 'missed') {
        toast?.info?.(curState === 'incoming' ? 'Пропущенный звонок' : 'Нет ответа');
      }
      cleanup('remote-end');
    };
  ...
  }, [cleanup, createPeerConnection, hangup, socket, sounds, toast]);

  // Страховка от «вечных гудков». Сервер финализирует непринятый звонок
  // через 30 с (callRegistry.PENDING_TIMEOUT_MS) и шлёт call:end — но если
  // это событие потерялось (обрыв сокета, рестарт сервера), окно звонка
  // висело бы бесконечно. Локальный таймер чуть больше серверного.
  const NO_ANSWER_TIMEOUT_MS = 35_000;
  useEffect(() => {
    if (state !== 'calling' && state !== 'incoming') return undefined;
    const t = setTimeout(() => {
  ...
  }, [state, cleanup, hangup, sounds, toast]);
  ...
  // Никакой персистенции звонка между перезагрузками. Раньше активный
  // звонок писался в sessionStorage и на следующем маунте восстанавливался
  // как 'waiting' — из-за этого после F5 висело мёртвое окно звонка.
  // Чистим возможные записи от старых версий клиента.
  useEffect(() => {
    try {
      sessionStorage.removeItem(ACTIVE_CALL_STORAGE_KEY);
    } catch {
      /* storage может быть отключён — не критично */
    }
  }, []);
  ...
  // мы СМОТРИМ во вкладку Sazcord. Возврат во вкладку сам по себе ctx
