...
export function AudioTab() {
  ...
  const requestPermission = async (kind: string) => {
    ...
    setPermissionError('');
    if ((window as any).electronAPI?.requestMediaPermission) {
      try {
        await (window as any).electronAPI.requestMediaPermission(kind);
      } catch {
        /* ignore */
      }
    }
  ...
  const startMicTest = async () => {
    try {
      if ((window as any).electronAPI?.requestMediaPermission) {
        try {
          await (window as any).electronAPI.requestMediaPermission('microphone');
        } catch {
          /* ignore */
        }
      }
